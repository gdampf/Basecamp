...
namespace {
  template < typename NAMEVALUETYPE >
    void debugPrint(std::ostream & stream, NAMEVALUETYPE & nameAndValue) {
      stream << nameAndValue -> name().c_str() << ": " 
	    << nameAndValue -> value().c_str();
    }
} // namespace

WebServer::WebServer(): events("/events"), server(80) {
...
void WebServer::begin(Configuration & configuration) {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest * request) {
    AsyncWebServerResponse * response =
	  request -> beginResponse_P(200, "text/html", index_htm_gz, index_htm_gz_len);
  ...
  server.on("/basecamp.css", HTTP_GET, [](AsyncWebServerRequest * request) {
    AsyncWebServerResponse * response =
	  request -> beginResponse_P(200, "text/css", basecamp_css_gz, 
	                             basecamp_css_gz_len);
    response -> addHeader("Content-Encoding", "gzip");
    request -> send(response);
  });
  ...
  server.on("/basecamp.js", HTTP_GET, [](AsyncWebServerRequest * request) {
    AsyncWebServerResponse * response =
	  request -> beginResponse_P(200, "text/js", basecamp_js_gz, basecamp_js_gz_len);
    response -> addHeader("Content-Encoding", "gzip");
    request -> send(response);
  });
  server.on("/logo.svg", HTTP_GET, [](AsyncWebServerRequest * request) {
    AsyncWebServerResponse * response = 
	  request -> beginResponse_P(200, "image/svg+xml", logo_svg_gz, logo_svg_gz_len);
    response -> addHeader("Content-Encoding", "gzip");
    request -> send(response);
  });

  server.on("/data.json", HTTP_GET, 
            [ & configuration, this](AsyncWebServerRequest * request) {
    AsyncJsonResponse * response = new AsyncJsonResponse(false, JSON_RESPONSE_SIZE);

    JsonVariant _jsonData = response -> getRoot();
    ...
    for (const auto & interfaceElement: interfaceElements) {
      ...
      for (const auto & attribute: interfaceElement.attributes) {
        attributes[attribute.first] = String {
          attribute.second
        };
      }
      ...
      if (interfaceElement.getAttribute("data-config").length() != 0) {
        if (interfaceElement.getAttribute("type") == "password") {
          ...
          attributes["value"] = String {
            configuration.get(interfaceElement.getAttribute("data-config"))
          };
        }
      }
    }
...
void WebServer::debugPrintRequest(AsyncWebServerRequest * request) {
  ...
  /**
  	...
  	*/
  const std::map < WebRequestMethodComposite, std::string > 
	requestMethods {
		{HTTP_GET, "GET"},
		{HTTP_POST, "POST"},
		{HTTP_DELETE, "DELETE"},
...
void WebServer::addInterfaceElement(const String & id, String element,
  String content, String parent, String configvariable) {
    interfaceElements.emplace_back(id, std::move(element), std::move(content), 
    std::move(parent));
...
void WebServer::setInterfaceElementAttribute(const String & id,
