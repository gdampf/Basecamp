...
if(!isset(DISPLAYS[$displayType])){
	echo ("Not a valid display size:".$displayType);
	exit;
}

$location = $_GET['loc'];
if(strlen($location) == 0) $location = "nk";
...
function rawImage($im) {
	...
	for ($y = 0; $y < imagesy($im); $y++) {
		for ($x = 0; $x < imagesx($im); $x++) {
			...
			$pixelcount++;
			if ($pixelcount % 8 == 0) {
				$byte = pack('H*', str_pad(base_convert($bits, 2, 16),2, "0", STR_PAD_LEFT));
				$bits = "";
				if ($byte == $lastbyte) {
				  $bytecount++;
				} else {
				  if ($bytecount != 0) {
				    if ($bytecount > 3 || $lastbyte == $escape) {
				      while ($bytecount > 0) {
				        $bytes .= $escape;
					$bytes .= $lastbyte;
					$bytes .= chr(($bytecount > 255)?0:($bytecount % 256));
				        $bytecount -= 256;
				      }
				    } else {
				      while ($bytecount-- > 0) $bytes .= $lastbyte;
				    }
				  }
				  $lastbyte = $byte;
				  $bytecount = 1;
