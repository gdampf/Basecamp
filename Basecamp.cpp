...
bool Basecamp::begin(String fixedWiFiApEncryptionPassword) {
  ...
  Serial.println("");
  Serial.println("Basecamp V.0.1.9 GD");
  ...
  if (shouldEnableConfigWebserver()) {
    ...
    // Add a webinterface element for the h1 that contains the device name. 
	// It is a child of the #wrapper-element.
    ...
    web.addInterfaceElement("infotext1", "p", 
	  "Configure your device with the following options:", "#wrapper");
    ...
    web.addInterfaceElement("DeviceName", "input", "Device Name", "#configform", 
	  "DeviceName");

    // Add an input field for the WIFI data and link it to the corresponding
    //	configuration data
    web.addInterfaceElement("WifiEssid", "input", "WIFI SSID:", "#configform", 
	  "WifiEssid");
    web.addInterfaceElement("WifiPassword", "input", "WIFI Password:", 
	  "#configform", "WifiPassword");
    web.setInterfaceElementAttribute("WifiPassword", "type", "password");
    web.addInterfaceElement("WifiConfigured", "input", "", "#configform", 
	  "WifiConfigured");
    ...
    if (configuration.get("MQTTActive") != "false") {
      web.addInterfaceElement("MQTTHost", "input", "MQTT Host:", "#configform", 
	    "MQTTHost");
      web.addInterfaceElement("MQTTPort", "input", "MQTT Port:", "#configform", 
	    "MQTTPort");
      ...
      web.addInterfaceElement("MQTTUser", "input", "MQTT Username:", "#configform", 
	    "MQTTUser");
      web.addInterfaceElement("MQTTPass", "input", "MQTT Password:", "#configform", 
	    "MQTTPass");
      web.setInterfaceElementAttribute("MQTTPass", "type", "password");
    }
    #endif

    // Add a save button that calls the JavaScript function collectConfiguration()
    // on click
    ...
    web.setInterfaceElementAttribute("footerlink", "href", 
    ...
    if (configuration.get("WifiConfigured") != "True") {
      ...
      xTaskCreatePinnedToCore( & DnsHandling, "DNSTask", 4096, 
	    (void * ) & dnsServer, 5, NULL, 0);
    }
    #endif
    #endif
  }
...
bool Basecamp::shouldEnableConfigWebserver() const {
  return (configurationUi_ == ConfigurationUI::always ||
    (configurationUi_ == ConfigurationUI::accessPoint && 
	 wifi.getOperationMode() == WifiControl::Mode::accessPoint));
}

#ifndef BASECAMP_NOMQTT

// This is a task that checks if the MQTT client is still connected or not.
// If not it automatically reconnect.
// TODO: Think about making void* the real corresponding type
void Basecamp::MqttHandling(void * mqttPointer) {
  // is set to true, when a connection attempt is already running.
...
// This function checks the reset reason returned by the ESP and resets the
// configuration if neccessary. It counts all system reboots that occured
// by power cycles or button resets. If the ESP32 receives an IP the boot
// counts as successful and the counter will be reset by Basecamps WiFi management.
void Basecamp::checkResetReason() {
  // Instead of the internal flash it uses the somewhat limited, but sufficient
  // preferences storage
  ...
  // If the reason is caused by a power cycle (1) or a RTC reset / button press(16)
  // evaluate the current bootcount and act accordingly.
  if (reason == 1 || reason == 16) {
    ...
    // If the counter is bigger than 3 it will be the fifths consecutive unsucessful
    // reboot.
    ...
    if (bootCounter > 3) {
      ...
      // Call the destructor for preferences so that all data is safely stored befor
      // rebooting
      preferences.end();
      ...
      // If the WiFi is unconfigured and the device is rebooted twice format the
...
String Basecamp::showSystemInfo() {
  ...
  if (configuration.isKeySet(ConfigurationKey::accessPointSecret)) {
    ...
    info << configuration.get(ConfigurationKey::accessPointSecret).c_str() 
	  << std::endl;
    info << "*******************************************" << std::endl;
  }
